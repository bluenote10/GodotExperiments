...
func get_normal(data, i, j, z_scale, grid_step_i, grid_step_j):
...
func coord(idx, axis_len, target_width):
    var scale = target_width / axis_len
    return (float(idx) - float(axis_len) / 2) * scale
...
func create_mesh(data, z_scale=3.0):
    ...
    var target_width_i = 10.0
    var target_width_j = len_j / 10.0
    var grid_step_i = target_width_i / len_i
    var grid_step_j = target_width_j / len_j
    
    var index = 0
    
    for i in len_i:
        for j in len_j:
            var x = coord(i, len_i, target_width_i)
            var z = coord(j, len_j, target_width_j)
            var y = z_scale * data[i][j]
            verts.append(Vector3(x, y, z))
            normals.append(get_normal(data, i, j, z_scale, grid_step_i, grid_step_j))
